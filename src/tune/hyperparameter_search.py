...
from QuarterCar_env.wrappers import ActionRepeat, RewardScaler, PreviewWrapper

_ALGO_CLS = {'td3': TD3, 'ppo': PPO}


def _make_env(road: str = 'speed_bump', seed: int = 0):
...
_SAMPLERS = {'td3': _sample_td3_params, 'ppo': _sample_ppo_params}
...
def _objective(trial: optuna.Trial, algo: str, n_timesteps: int) -> float:
    ...
    run_env.close()
    return float(sum(episode_rewards) / len(episode_rewards))


def main():
    parser = argparse.ArgumentParser(description='Optuna HP search for COptRL')
    parser.add_argument('--algo',       default='ppo', choices=['td3', 'ppo'])
    ...
    study_name = args.study_name or f'{args.algo}_COptRL'
