...
def compute_reward(
    ...
    Jjerk = r_jerk(filtered_jerk, cfg.j_max, cfg.reward_jerk_clip)
    Jsmooth = r_action_smooth(action, prev_action)
    jerk_smooth_penalty = cfg.w_jerk * Jjerk + cfg.w_action_smooth * Jsmooth
    ...
    bd = {
        ...
        "J_jerk":   float(np.nan_to_num(Jjerk,  nan=0.0)),
        "J_smooth": float(np.nan_to_num(Jsmooth, nan=0.0)),
