...
def r_speed_band(v: float, v_min: float, v_limit: float) -> float:
    # penalise distance from v_limit in both directions; extra penalty below v_min
    # NOT velocity-scaled: a scaled penalty at v≈0 vanishes → stop-and-wait exploit
    if v < v_min:
        return -1.0 - ((v_min - v) / v_min) ** 2
    return -((v_limit - v) / v_limit) ** 2
...
def compute_reward(
    ...
    rt = r_speed_band(v, cfg.v_min, cfg.v_limit)
    # NOT velocity-scaled — see r_speed_band docstring
    ...
    # jerk and action_smooth intentionally NOT velocity-scaled:
    # they measure self-induced oscillation the agent fully controls
